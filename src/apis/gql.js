...
function cleanObject (arg) {
    ...
    const entries = Object.entries(arg).map(([key, value]) => {
        switch (typeof (value)) {
        case 'number': return [key, Number.isNaN(value) ? null : value]
        case 'string': return [key, cleanString(value)]
        case 'object': return [key, Array.isArray(value) ? cleanArray(value) : cleanObject(value)]
